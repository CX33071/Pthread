...
class Task {
   public:
    ...
    void set(functype func, void* arg) { 
        func_ = func;
         arg_ = arg;
     }
     void exe() { func_(arg_); }
...
class Queue {
    public:
        ...
        bool add_task(Task& task) { std::lock_guard<std::mutex> lock(mutex_);
            if(count_>=MAX_NUM){
                return false;
            }
            tasks[last_] = task;
        ...
        bool gettask(Task&task,bool is_running) { std::unique_lock<std::mutex> lock(mutex_); 
